...
class Image extends Component
{
	...
	public $hasDifferentMobileImage;
     ...
     * @return void
     */
    public function __construct(mixed $image = null, string $size = 'large', string|null $class = null, string|null $alt = null, bool $lazy = true, bool $container = false, bool|null $hasDifferentMobileImage = false, mixed $mobileImage = null, bool|null $centerImage = false)
    {
        $this->attr = [];
		...
		$this->hasDifferentMobileImage = $hasDifferentMobileImage;

		$this->mobileImage = $this->display_image($mobileImage, $size, $this->attr);
    ...
    public function display_image($image, $image_size, $attr = '', $inline_svg = false)
    {
        ...
        if (is_numeric($image) && wp_attachment_is_image($image)) {
            $default_attr['alt'] = trim(wp_strip_all_tags(get_post_meta($image, '_wp_attachment_image_alt', true)));
        }
